...
namespace Ecommerce.WebAssembly.Servicios.Implementacion
{
    public class VentaServicio: IVentaServicio
    {
        ...
        public async Task<ResponseDTO<VentaEcommerceDTO>> Registrar(VentaEcommerceDTO modelo)
        {
            var response = await _httpClient.PostAsJsonAsync("Venta/Registrar", modelo);
            var result = await response.Content.ReadFromJsonAsync<ResponseDTO<VentaEcommerceDTO>>();
            return result;
        }
    }
}
